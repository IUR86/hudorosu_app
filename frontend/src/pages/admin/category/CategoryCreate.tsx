...
export const CategoryCreatePage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.response?.data?.error || 'カテゴリーの作成に失敗しました');
        } finally {
            setIsSubmitting(false);
        }
    };
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <nav className="flex items-center text-sm text-slate-500 gap-2 mb-2 font-medium tracking-wide">
                                ...
                                <span className="text-slate-900 font-bold">新規カテゴリ作成</span>
                            </nav>
                            <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">新規カテゴリ作成</h2>
                        </div>
                    </div>
